...
CJ · 경동 배송을 확인하고 출고하세요.
  ...
  `}function x(){let e=u().length;return b({id:`shipButton`,text:e>0?`선택 ${e}건 출고완료`:`선택 0건`,variant:e>0?`primary`:`disabled`,disabled:e===0})}function S(){document.querySelector(`#appScreen`).innerHTML=y({bottomAction:x()}),C(),T(),w(),E()}function C(){document.querySelectorAll(`.order-card`).forEach(e=>{e.addEventListener(`click`,e=>{let t=e.target.closest(`.order-card`);t&&(f(t.dataset.orderId),S())})})}function w(){let e=document.querySelector(`#searchInput`);e&&e.addEventListener(`keydown`,e=>{e.key===`Enter`&&(p(e.target.value),S())})}function T(){document.querySelectorAll(`.filter-chip`).forEach(e=>{e.addEventListener(`click`,()=>{m(e.dataset.filter),S()})})}function E(){let e=document.querySelector(`#shipButton`);e&&e.addEventListener(`click`,async()=>{try{if(e.disabled=!0,e.textContent=`출고 처리중...`,await h()===0)return;S()}catch(e){alert(`출고완료 처리 실패`),console.error(e),S()}})}function D(){return`
    <div class="app-shell">
